...
cycle_palette_anim:
        ...
        ;cycles the palette
        mov     dl,[bottom_palette_tbl+1]       ;save first value
        mov     cx,14                           ;shift table to the left once
        mov     si,bottom_palette_tbl+2         ; but don't touch color black
        mov     di,bottom_palette_tbl+1         ; needed to avoid flicker
        rep     movsb
        mov     [bottom_palette_tbl+15],dl      ;color[15] = color[1]
...
;=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-;
palette_colors_init:
        mov     byte [top_palette_tbl+0],1      ;black in blue
        mov     byte [bottom_palette_tbl+0],1   ;black is blue
        ret
...
.loop:
        ...
        mov     [top_palette_tbl+di],al         ;update color in color table
        loop    .loop                           ; and loop

        mov     [bottom_palette_tbl+0],al       ;update black for bottom part as well
...
.loop:
        ...
        mov     byte [top_palette_tbl+bx],dl    ;update color index with blue
...
state_pvm_logo_fade_in_anim:
        ...
        mov     [top_palette_tbl+2],al          ;update color index 2


        ;letter V
        sub     bx,8                            ;offset -8
        mov     al,[palette_pvm_logo_fade_tbl+bx]       ;fetch color value
        mov     [top_palette_tbl+0xa],al        ;update color index 0xa

        ;letter M
        sub     bx,8                            ;offset -8
        mov     al,[palette_pvm_logo_fade_tbl+bx]       ;fetch color value
        mov     [top_palette_tbl+0xd],al        ;update color index 0xd
...
plasma_to_video_anim:
        ...
        ; do plasma
        mov     bx,[luminances_addr]            ;to be used by xlat. has the colors for the plasma
        ...
        mov     word [plasma_x_offset],0
        call    plasma_init_sine_table

        mov     al,[luminance_idx]
        inc     al
        cmp     al,LUMINANCES_MAX
        jne     .skip
        sub     al,al
.skip:
        mov     [luminance_idx],al
        shl     al,1
        sub     ah,ah

        mov     bx,ax
        mov     cx,[luminances_addrs+bx]
        mov     [luminances_addr],cx
        ret
.minus_1:
...
luminances_addr:                                                ;use in plasma anim
        dw      0                                               ;faster to have it pre-calculated
luminance_idx:                                                  ;which of the luminances addr should use
        db      0
luminances_addrs:                                               ;the possible palettes to use for the plasma
;        dw      luminances_8_color_lo
        dw      luminances_6_red_colors
;        dw      luminances_8_color_hi
        dw      luminances_6_blue_colors
        dw      luminances_6_green_colors
LUMINANCES_MAX equ ($-luminances_addrs)/2
