...
;=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-;
plasma_init:
        mov     byte [plasma_state],0
        mov     word [plasma_x_offset],0
        ret

;=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-;
PLASMA_X equ 2                                 ;plasma: pixels wide
PLASMA_Y equ 32                                  ;plasma: pixels height
PLASMA_OFFSET equ 21*2*160+158                 ;plasma: video offset

plasma_anim:
        cmp     byte [plasma_state],0
        je      plasma_to_buffer_anim
        jmp     plasma_to_video_anim

;=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-;
plasma_to_video_anim:
        dec     byte [plasma_state]             ;prev state for the plasma

        %assign YY 0
        %rep    PLASMA_Y

                mov     cx,PLASMA_X/2                   ;moving words, instead of bytes. div by 2
                mov     si,plasma_buffer+PLASMA_X*YY    ;points to next row in plasma buffer
                mov     di,PLASMA_OFFSET+(YY/4)*160+(YY % 4)*8192 ;points to next row in video buffer
                add     di,[plasma_x_offset]
                rep movsw                               ;copy ds:si -> es:di
        ...
        ret
.minus_1:
        sub     word [plasma_x_offset],1
        ret

;=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-;
plasma_to_buffer_anim:
        inc     byte [plasma_state]             ;next state for the plasma
        ...
        %rep    PLASMA_X
                ...
                add     bl,7                    ;update offsets to sine tables
                add     si,9
        ...
        %rep    PLASMA_Y
                ...
                %rep    PLASMA_X
                        ...
                        mov     [plasma_buffer+YY*PLASMA_X+XX],al       ; and plasma buffer. 2 pixels per plasma point. cheaper.
...
sine_ybuf_2_idx:                                ;plasma: ybuf idx 2
        db      0
plasma_state:                                   ;plasma state machine
        db      0
...
plasma_ybuf:                                    ;plasma ybuffer
        resb   PLASMA_Y
plasma_buffer:                                  ;where the plasma will be pasted in the first pass
        resb    PLASMA_Y * PLASMA_X
sine_table:
